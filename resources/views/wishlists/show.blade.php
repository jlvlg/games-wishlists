<x-app-layout>
    <x-slot name="header">
        ...
        <form :action="route('wishlists.index')" class="flex items-center flex-1 rounded-lg bg-gray-800 text-gray-400 border border-gray-500 mx-5" method="get">
            ...
            <input type="search" class="flex-1 border-none bg-transparent" name="query" id="search" placeholder="Search games">
            ...
            <a class="mr-3 {{(request()->filled('query')) ? '' : 'hidden'}}" href="{{route('wishlists.show', ['id' => $wishlist->id])}}"><svg class="fill-gray-400" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="m12 10.93 5.719-5.72c.146-.146.339-.219.531-.219.404 0 .75.324.75.749 0 .193-.073.385-.219.532l-5.72 5.719 5.719 5.719c.147.147.22.339.22.531 0 .427-.349.75-.75.75-.192 0-.385-.073-.531-.219l-5.719-5.719-5.719 5.719c-.146.146-.339.219-.531.219-.401 0-.75-.323-.75-.75 0-.192.073-.384.22-.531l5.719-5.719-5.72-5.719c-.146-.147-.219-.339-.219-.532 0-.425.346-.749.75-.749.192 0 .385.073.531.219z"/></svg></a>
        </form>
    ...
    <div class='text-gray-400'>
        @foreach ($games as $game)
            <div class='flex gap-2 my-3 mx-5'>
                <x-button-link class='flex-1 bg-gray-700 hover:bg-gray-600 focus:bg-gray-500 transition text-xl max-w-full truncate' :href="route('games.show', ['id' => $game->id])">
